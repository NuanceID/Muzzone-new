  ...
  Widget build(BuildContext context) {
    return MultiRepositoryProvider(
        ...
        child: MultiBlocProvider(providers: [
          ...
          BlocProvider<SlidingUpPanelBloc>(
              create: (buildContext) =>
                  SlidingUpPanelBloc(panelController: PanelController())),
          ...
          BlocProvider<GenresBloc>(
              create: (buildContext) => GenresBloc(
                  backendRepository: buildContext.read<BackendRepository>())),
          BlocProvider<PlaylistsBloc>(
              create: (buildContext) => PlaylistsBloc(
                  backendRepository: buildContext.read<BackendRepository>())),
          BlocProvider<CategoriesBloc>(
              create: (buildContext) => CategoriesBloc(
                  backendRepository: buildContext.read<BackendRepository>())),
          BlocProvider<SlidingUpPanelPositionBloc>(
              create: (buildContext) => SlidingUpPanelPositionBloc()),
          BlocProvider<BottomNavigationBarBloc>(
              create: (buildContext) => BottomNavigationBarBloc()),
          BlocProvider<RecentTracksBloc>(
            create: (BuildContext context) =>
                RecentTracksBloc()..add(const GetRecentTracks()),
          ),
          BlocProvider<MyPlayListsBloc>(
            create: (BuildContext context) =>
                MyPlayListsBloc()..add(const GetMyPlayLists()),
          )
        ], child: child));
  }
}
